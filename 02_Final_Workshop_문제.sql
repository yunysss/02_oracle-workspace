...
--4
SELECT WRITER_NM, MOBILE_NO, HOME_TELNO
  FROM (SELECT *
          FROM TB_WRITER
...
SELECT WRITER_NM
  FROM TB_WRITER
 WHERE TO_NUMBER(TO_DATE('06/01/31') - REGIST_DATE) / 365 >= 31
ORDER BY 1;
...
SELECT BOOK_NM "도서명", STOCK_QTY "재고수량", PRICE "가격(Org)", PRICE * 0.8 "가격(NEW)"
  FROM TB_BOOK
 WHERE TO_NUMBER(SYSDATE - ISSUE_DATE) / 365 >= 30
   AND STOCK_QTY >= 90
ORDER BY 2 DESC, PRICE * 0.2 DESC, 1;
